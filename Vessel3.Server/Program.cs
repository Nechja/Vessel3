...
var fsyncThreads = int.TryParse(Environment.GetEnvironmentVariable("VESSEL3_FSYNC_THREADS"), out var ft) && ft > 0 ? ft : 4;
builder.Services.AddSingleton(new DurabilityWriter(fsyncThreads));
builder.Services.AddSingleton<IDurabilityWriter>(sp => sp.GetRequiredService<DurabilityWriter>());
builder.Services.AddHostedService(sp => sp.GetRequiredService<DurabilityWriter>());
